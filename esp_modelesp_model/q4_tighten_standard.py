...
def run(data_path: str, out_dir: str, k: int, grid_n: int):
    ...
    for c in sorted(dff["cluster"].unique()):
        ...
        e10 = search_cluster(sub, model, feats, 10.0, grid_n)
        e5 = search_cluster(sub, model, feats, 5.0, grid_n)
        if e10 is None or e5 is None:
            continue
        rows.append({"cluster": int(c), "energy_10": e10, "energy_5": e5, "increase_pct": (e5-e10)/e10*100})
    res = pd.DataFrame(rows)
    res.to_csv(f"{out_dir}/q4_energy_increase_5mg.csv", index=False)
    if len(res) > 0:
        pd.DataFrame([{"avg_increase_pct": res["increase_pct"].mean()}]).to_csv(f"{out_dir}/q4_summary.csv", index=False)
